...
static void draw_devices(
  ...
  unsigned double percentage_gpu_power = 0;
  ...
  for (unsigned int i = 0; i < num_devices; ++i) {
    ...
    */
    ...
    if (IS_VALID(power_draw_valid    , dinfo->valid) &&
      ...
      percentage_gpu_power = (unsigned int)(dinfo->power_draw_max / dinfo->power_draw_max_card) * 100;
      //Print
      ...
      if((dinfo->power_draw/1000) >= (dinfo->power_draw_max/1000 - 15) && (dinfo->power_draw/1000) < dinfo->power_draw_max/1000) {
        mvwchgat(dev->power_info, 0, 0, 4, 3, 0, yellow_color, NULL);
      } else if((dinfo->power_draw/1000) >= (dinfo->power_draw_max/1000)) {
        mvwchgat(dev->power_info, 0, 0, 4, 3, 0, red_color, NULL);
      }
