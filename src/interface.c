...
static void draw_devices(
  ...
  unsigned int percentage_gpu_power_draw = 0;
  ...
  for (unsigned int i = 0; i < num_devices; ++i) {
    ...
    */
    ...
    if (IS_VALID(gpu_clock_speed_valid, dinfo->valid)) {
      ...
      if(dinfo->gpu_clock_speed >=  dinfo->gpu_clock_speed_max - 200 && dinfo->gpu_clock_speed <  dinfo->gpu_clock_speed_max) {
        //If the GPU core clock is 200 Mhz less than MAX GPU Mhz set text yellow 
    ...
    if (IS_VALID(mem_clock_speed_valid, dinfo->valid)) {
      ...
      //Change text color if limits are passed
      if(dinfo->mem_clock_speed >=  dinfo->mem_clock_speed_max - 200 && dinfo->mem_clock_speed <  dinfo->mem_clock_speed_max) {
        //If the GPU memory clock is 200 Mhz less than MAX GPU Mhz set text yellow 
    ...
    if (IS_VALID(power_draw_valid    , dinfo->valid) &&
      ...
      //Percentage of power draw
      percentage_gpu_power_draw = (((double)dinfo->power_draw/1000)/((double)dinfo->power_draw_max_card/1000))*100;
      ...
      //Print
      mvwprintw(dev->power_info, 0, 0, "POW %3u/%3u/%3u/%4uW @%3u/%3u%%", dinfo->power_draw / 1000, dinfo->power_draw_max / 1000, dinfo->power_draw_max_card / 1000, total_gpu_power, percentage_gpu_power_draw, percentage_gpu_power);
