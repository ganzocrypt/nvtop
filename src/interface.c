  ...
  for (unsigned int i = 0; i < num_devices; ++i) {
    ...
    */
    ...
    if (IS_VALID(power_draw_valid    , dinfo->valid) &&
      ...
      percentage_gpu_power = 100 * (dinfo->power_draw_max/1000 / dinfo->power_draw_max_card/1000);
      //Print
      mvwprintw(dev->power_info, 0, 0, "POW %3u/%3u/%3u/%4uW @%3u%%", dinfo->power_draw / 1000, dinfo->power_draw_max / 1000, dinfo->power_draw_max_card / 1000, total_gpu_power, dinfo->power_draw_max / dinfo->power_draw_max_card * 100);
